...
class formulizeAdvancedCalculation extends icms_core_Object {
...
class formulizeAdvancedCalculationHandler {
	...
	function insert(&$advCalcObject, $force = false) {
		...
		if (!$result) {
			print "Error: this advanced calculation could not be saved in the database.  SQL: $sql<br>" . icms::$xoopsDB->error();
			return false;
		}
	...
	function createLog($acid, $uid) {
		...
		if (!$result) {
			print "Error: this advanced calculation log could not be saved in the database.  SQL: $sql<br>" . icms::$xoopsDB->error();
			return false;
		}
	...
	function createLogParam($logid, $param, $value) {
		...
		if (!$result) {
			print "Error: this advanced calculation log item could not be saved in the database.  SQL: $sql<br>" . icms::$xoopsDB->error();
			return false;
		}
	...
	function getNextAgeRange($ageRange, $minMax, $boundaryValue) {
		$keyRange = array("<1", "1-5", "6-12", "13-15", "16+");
		$mins = array(0, 1, 6, 13, 16);
		$maxes = array(0.999, 5.999, 12.999, 15.999, 99);
		switch ($minMax) {
			case "min":
...
function reportProceduresTime($label, $averageOverThisNumber = 0) {
	if (!isset($GLOBALS['formulize_procedureTimerOn'])) {
		return;
	}
	static $time;
	static $totalTime;
...
function createProceduresTable($array, $permTableName = "") {
	...
	foreach ($array as $fieldData) {
		...
		if ($thisDataMultipleCount > 1) {
			foreach ($fieldList as $fieldName) {
				if ($fieldName == $thisDataMultipleField) {
					continue;
				}
