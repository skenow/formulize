...
class formulizeFormsHandler {
	...
	function &get($fid, $includeAllElements = false) {
		...
		if (isset($cachedForms[$fid][$includeAllElements])) {
	...
	function getByHandle($handle) {
		global $xoopsDB;
		$sql = "SELECT id_form FROM " . $xoopsDB->prefix("formulize_id") . " WHERE form_handle = '" . icms::$xoopsDB->escape($handle) . "'";
	...
	function isHandleUnique($handle, $element_id = "") {
		...
		$sql = "SELECT count(ele_handle) FROM " . $xoopsDB->prefix("formulize") . " WHERE ele_handle = '" . icms::$xoopsDB->escape($handle) . "' $element_id_condition";
		if (!$res = $xoopsDB->query($sql)) {
	...
	function createDataTable($fid, $clonedForm = 0, $map = false, $revisionsTable = false) {
		...
		foreach ($formObject->getVar('elementHandles') as $elementId => $thisHandle) {
			...
			if ($elementTypes[$elementId] == "areamodif" or $elementTypes[$elementId] == "ib" or $elementTypes[$elementId] == "sep" or $elementTypes[$elementId] == "grid" or $elementTypes[$elementId] == "subform") {
				continue;
			} // do not attempt to create certain types of fields since they don't live in the db!
	...
	function setPerGroupFilters($filterSettings, $fid) {
		...
		foreach ($filterSettings as $groupid => $theseSettings) {
			if (isset($foundGroups[$groupid])) {
				// add to update query
				$updateSQL .= "WHEN $groupid THEN '" . icms::$xoopsDB->escape(serialize($theseSettings)) . "' ";
				$runUpdate = true;
			} else {
				...
				$insertSQL .= "(" . $fid . ", " . $groupid . ", '" . icms::$xoopsDB->escape(serialize($theseSettings)) . "')";
				$insertStart = false;
				$runInsert = true;
			}
		}
	...
	function clearPerGroupFilters($groupids, $fid) {
		...
		global $xoopsDB;
		$deleteSQL = icms::$xoopsDB->escape("DELETE FROM " . $xoopsDB->prefix("formulize_group_filters") . " WHERE fid=$fid AND groupid IN (" . implode(", ", $groupids) . ")");
	...
	function buildPerGroupFilterWhereClause($match, $indexes, $filterSettings, $uid, $formAlias) {
		...
		for ($io = 0; $io < count($indexes); $io++ ) {
			...
			$termToUse = (is_numeric($termToUse) and !strstr(strtoupper($filterSettings[1][$i]), "LIKE")) ? $termToUse : "\"$likeBits" . icms::$xoopsDB->escape($termToUse) . "$likeBits\"";
	...
	function cloneForm($fid, $clonedata = false) {
		...
		foreach ($getrow[0] as $field => $value) {
			...
			if ($field == "headerlist" or $field == "defaultform" or $field == "defaultlist") {
				$value = "";
			}
