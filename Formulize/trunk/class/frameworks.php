...
class formulizeFramework extends icms_core_Object {
	...
	function __construct($frid = "") {
		...
		if ($notAFramework) {
			list($frid, $fids, $name, $handles, $element_ids, $links, $formHandles) = $this->initializeNull();
		}
...
class formulizeFrameworkLink extends icms_core_Object {
...
class formulizeFrameworksHandler {
	...
	function insert(&$framework) {
		if (!is_object($framework) or get_class($framework) != 'formulizeFramework') {
			return false;
		}
